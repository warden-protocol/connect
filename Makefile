...
HTTPS_GIT := https://github.com/warden-protocol/connect.git
...
BUILD_TAGS := -X github.com/warden-protocol/connect/cmd/build.Build=$(TAG)
...
format:
	...
	@find . -name '*.go' -type f -not -path "*.git*" -not -path "/*mocks/*" -not -name '*.pb.go' -not -name '*.pulsar.go' -not -name '*.gw.go' | xargs go run golang.org/x/tools/cmd/goimports -w -local github.com/warden-protocol/connect
